...
from prompt_ensemble import SupFAD_PromptLearner
...
def test(args):
    ...
    # 设置SupFAD模型参数
    SupFAD_parameters = {"Prompt_length": args.n_ctx, "learnabel_text_embedding_depth": args.depth, "learnabel_text_embedding_length": args.t_n_ctx}
    
    # 加载模型
    model, _ = SupFAD_lib.load("ViT-L/14@336px", device=device, design_details = SupFAD_parameters)
    model.eval()

    preprocess, target_transform = get_transform(args)

    # 初始化提示学习器
    prompt_learner = SupFAD_PromptLearner(model.to("cpu"), SupFAD_parameters)
    ...
    with torch.no_grad():
        ...
        for idx, patch_feature in enumerate(patch_features):
            if idx >= args.feature_map_layer[0]:
                ...
                similarity, _ = SupFAD_lib.compute_similarity(patch_feature, text_features[0])
                similarity_map = SupFAD_lib.get_similarity_map(similarity[:, 1:, :], args.image_size)
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser("SupFAD", add_help=True)
    # 路径参数
